...
- **Semantic search** via Pinecone + Azure OpenAI embeddings
...
```env
# Semantic Search (Pinecone) - Optional but recommended
PINECONE_API_KEY=your_key

# Embeddings (choose one) - Optional
...
Without these, server uses local TF-IDF search (still good).
...
*This document ensures AI session continuity. Update it when major changes are made.*
